...
public class PlayerMove : MonoBehaviour
{
    ...
    public void Destroy_Wall(float x, float y) // 벽 파괴 함수
    {
        if (x == 0) // 상하 이동
        {
            ...
            {
                if (overCollider2d.gameObject.layer == LayerMask.NameToLayer("Wall"))
                {
                    overCollider2d.transform.GetComponent<Wall>().Pullwall(Destroy_Position, x, y); // 벽이 있으면 밀기
                }
                else if (overCollider2d.gameObject.layer == LayerMask.NameToLayer("BrokenWall"))
                {
                    overCollider2d.transform.GetComponent<BrokenWall>().PullBrokenwall(Destroy_Position, x, y); // 벽이 있으면 밀기
                }
            }
